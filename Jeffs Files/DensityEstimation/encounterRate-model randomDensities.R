...
cat("
    ...
    model{
      ...
      er.mean  ~ dnorm(3.13,tau.er)     # r, the effective search radius (3.13 km), from Jay's max sim lik model (for 4km truncation, Jan 2021)
      tau.er <- 1/(sd.er * sd.er)
      sd.er <- 0.313                    # Jay provided CV = 0.10 (so SD = 0.313)
      ...
      s ~ dnorm(1.9, tau.s)           # group size.  Mean s comes from Barlow 2016 report
        tau.s <- 1/(sd.s * sd.s)
        sd.s <- 0.13

      # from EH analysis... number of minutes that a beaker is available for detection...
      timeAvail ~ dnorm(17.23, tau.timeAvail)
      ...
      deepDivePeriod ~ dnorm(191.4,ddp.tau)
      ddp.tau <- 1/(ddp.sd * ddp.sd)
      ddp.sd <- 28.2
