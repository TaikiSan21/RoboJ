...
source("SimulatedLikelihoodFunctions v7.R")
...
####################################################################################################
  ...
  globalVariables(c("gam_logLike","Param1Limits","Param2Limits","truncDist","TruncAngle"))
  SpeciesID<- "ZC"
  SubSetNames<- c("All Recorders","SM3M Recorders","SM2 Recorders","ST4300 Recorders")
  ...
  #actual truncation angle based on nominal truncation dist. and mean depth
  TruncAngle<- atan(truncDist/(meanDepth-HP_depth))*180/pi  
  SD_angleErr<- 0.59    #standard deviation in angle measurements from mean accelerometer value
  DepthDistr<- "log-normal"  #use log-normal distribution for depths (alternative is "normal")
  Range<- seq(1000,3000,10)   # range to search likelihoods for P1 in det function
  JK_nsamples<- 20
  #Model<- "HN"       # half-normal detection model
  Model<- "C_HN"     #compound half-normal detection mdoel
  UseSimulatedData<- FALSE     # If false use real data
  if (!UseSimulatedData) {     #get click angle data and subset it
    ...
    ClickAngles<- ClickAngles[(ClickAngles$nClicks>2),]   
    BeakedWhales<- ClickAngles[ClickAngles$eventType %in% c("BB","BW","BW26-47","BW38",
                                                            "BW39V","BW43","BWunid","MS","ZC"),]
    cat(" Tot number of files w/ all beaked whale species and 3 or more clicks = ",length(ClickAngles$EventId))
    # Limit sample to SpeciesID
    ClickAngles<- ClickAngles[ClickAngles$eventType == SpeciesID,]
  }
...
####################################################################################################
  if (UseSimulatedData) {
    ...
    if (SubSetName == "SM3M Recorders") {
      ClickAngleSubset<- ClickAngles[ClickAngles$Recorder == "SM3M",]  
    } else if (SubSetName == "SM2 Recorders") {
      ClickAngleSubset<- ClickAngles[ClickAngles$Recorder == "SM2Bat",]  
    } else if (SubSetName == "ST4300 Recorders") {
      ClickAngleSubset<- ClickAngles[ClickAngles$Recorder == "ST4300",]  
    } else {ClickAngleSubset<- ClickAngles}

    DriftsWithDetection<- unique(ClickAngleSubset$Station)
    nDrifts<- length(DriftsWithDetection)
    # Observed detection angles are valid average angles 
    ObsDetAngles<- ClickAngleSubset$MeanAngle[!is.na(ClickAngleSubset$MeanAngle)]   
    ...
    histout<-hist(ClickAngles$MeanAngle,breaks=50,col="black",xlab="Detection Angle (deg)",
...
####################################################################################################
  ...
  if (Model == "HN") {          #univariate fit
    gam_logLike<<- gam(formula= LogLike ~ te(Param1,k=50), data=LogLikeDF)
    plot(Range,gam_logLike$fitted.values,type="l",ylab="Log-Likelihood")
  }else{                        #bivariate fit
    gam_logLike<<- gam(formula= LogLike ~ te(Param1,Param2,k=50), data=LogLikeDF)
    plot(gam_logLike)
  }
...
####################################################################################################
  if (Model == "C_HN") {
    ...
    FindMaxLik<- optim(par=ParGuess,fn=SmoothLogLikelihood,method="Brent",upper=4000,lower=500,
                      hessian= TRUE,control=list(trace=5,fnscale=-1)) 
  }
####################################################################################################
